...
**Scope change, 11 September 2026.** The project now covers only fragile tokens: detecting them
from embeddings, verifying them with agent tool-call tasks, and analysing the output embeddings.
All training-dynamics code was removed that day: the from-scratch rig (`src/rig/`), the pretraining
trajectory and stage-relative abandonment analyses, the day-one weld and repetition experiments, and
the ad-hoc scripts in `results/remote_root/`. Their result files (`results/traj_*.pt`, `results/f2_*`,
`results/sw_*`, `results/day1*`, `runs/`, and the rig corpora in `data/`) are still on disk but have no
code that regenerates them. The rows for those sections are gone from the table below.
...
- `results/remote_root/`: root-level logs from the earliest phases (`corpus.log`,
  `prep.log`, `families.log`, `fam2.log`, `program.log`), `cf.txt` and
  `results_candidates.json`. Its ad-hoc scripts and the `src_remote.tgz` snapshot of the
  remote `src/` were removed with the training-dynamics code; the local `src/` is canonical.
- `logs/`: all run logs. These are the authoritative record of printed
...
- `results/remote_h100/`: snapshot of the node's results, docs and logs (including
  `logs/env_h100_*` — driver 580.105.08, CUDA 13.0, torch 2.14.0, transformers 5.16.1,
  pip freeze). The 1x H100 was fully extracted 6 September 2026 20:45 UTC. Its copy of `src/`
  and its staged scripts were removed on 11 September 2026: the canonical code is `src/`, and the
  staged scripts `rerun_17b.sh`, `greedy_specimens.sh` and `bootstrap.sh` are in `logs/`.
