...
echo "#### 7d. Qwen3.8-Flash-Next-FP8 rung (v6, 56GiB/card cap + fp8 embedding rescale + indexer fast path), N_TOK=8400, chat framing, teacher-forced only  ($(date -u +%H:%M:%S))" | tee -a logs/scale.log
...
grep -E "QSA indexer|fp8 embedding|filler pool|single probe" logs/${T}_fragility.log | tee -a logs/scale.log
...
echo "QWEN38 FLASH DONE4 $(date -u)" | tee -a logs/scale.log
